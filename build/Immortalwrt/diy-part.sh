...
export DNS_Settings="119.29.29.29 223.5.5.5"                      # 旁路由设置 DNS(填入DNS，多个DNS要用空格分开)(填0为不作修改)
...
export Customized_Information="$(TZ=UTC-8 date "+%Y-%m-%d") Bulid By Aihdde"  # 个性签名,你想写啥就写啥，(填0为不作修改)
...
grep -rl 'Vlmcsd KMS 服务器' . | xargs -r sed -i 's?Vlmcsd KMS 服务器?KMS激活?g'
